...
for (var i = 0; i < users.length; i++ ){
 ...
 var GuessRatio = JSON.stringify(users[i].username) + JSON.stringify((Math.round(users[i].correctGuesses / users[i].guesses * 100)))
 document.write(GuessRatio);
...
function sortArray() {
  GuessRatio.sort(function(a, b){return b - a});
  document.getElementById("highscore").innerHTML = GuessRatio;
}
}
